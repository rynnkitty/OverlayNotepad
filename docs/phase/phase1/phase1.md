...
> **Status**: Sprint 1 완료 (2026-04-17) — Sprint 2 진행 예정
...
| 2 | 투명도 모드 + Always on Top + 컨텍스트 메뉴 골격 | 배경/전체 투명 모드, 투명도 단계 조절, Topmost 토글, ContextMenu | Sprint 1 | 예정 |

---

## Sprint 1 상세 — WPF 프로젝트 구조 + 투명 윈도우 + 기본 텍스트 입력 ✅ 완료

> PR: phase1-sprint1 → develop (2026-04-17 완료, MSBuild 0 Errors + 수동 검증 전 항목 통과)
...
| Background Transparent 히트 테스트 명확화 | 낮음 (Medium) | 드래그 Border의 `Background="Transparent"`는 히트 테스트 통과 여부가 구현 방식에 따라 다름. 현재 동작은 검증 완료되었으나, 명시적으로 `Background="#00FFFFFF"` (알파=0 색상값)으로 변경하면 의도가 더 명확함. Sprint 1에서 실기능 문제 없음으로 확인. | Sprint 2에서 개선 권장 — DragArea Border Background를 `#00FFFFFF`로 변경 |
...
| 항목 | 기준 | Sprint | 상태 |
|------|------|--------|------|
| 투명 윈도우 | 타이틀바 없는 투명 윈도우에서 드래그 이동 및 리사이즈 동작 | Sprint 1 | ✅ 완료 |
| 텍스트 입력 | 투명 배경 위에서 한글/영문 텍스트 입력 정상 동작 | Sprint 1 | ✅ 완료 |
| 표준 편집 | 복사/붙여넣기, Undo/Redo 등 표준 편집 기능 동작 | Sprint 1 | ✅ 완료 |
| 자동 포커스 | 실행 즉시 TextBox에 포커스, 바로 타이핑 가능 | Sprint 1 | ✅ 완료 |
| 투명 모드 | 배경 투명 / 전체 투명 모드 전환 동작 | Sprint 2 | ⬜ 대기 |
| 투명도 조절 | 컨텍스트 메뉴에서 투명도 단계별 조절 동작 | Sprint 2 | ⬜ 대기 |
| Always on Top | Topmost 토글 동작 (기본값: 활성화) | Sprint 2 | ⬜ 대기 |
| 컨텍스트 메뉴 | 우클릭 메뉴에서 투명도, 모드 전환, Always on Top, 종료 동작 | Sprint 2 | ⬜ 대기 |
| 메모리 | 유휴 상태 메모리 사용량 80MB 이하 | Sprint 2 | ⬜ 대기 |
