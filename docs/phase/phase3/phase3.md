...
| FontDialog/ColorDialog Dispose 미호출 | ⚠️ 낮음 | Sprint 1 → Sprint 2에서 개선 권장 | MainWindow.xaml.cs FontDialog_Click, ColorDialog_Click에서 `using` 블록 또는 명시적 `Dispose()` 호출로 개선. WinForms GC 위임 일반적이나 명시적 해제 권장 |
| XAML 초기 IsChecked와 settings.json 불일치 가능성 | ⚠️ 낮음 | Sprint 1 → Sprint 2에서 개선 권장 | MainWindow.xaml의 `BackgroundTransparentMenuItem IsChecked="True"` 하드코딩. Window_Loaded에서 settings 로드 후 동기화 로직 추가 권장 |
...
| 다크 테마 | 글자 #FFFFFF, 배경 #1E1E1E/80%, 테두리 #000000 적용 | ⬜ (Sprint 2) |
| 라이트 테마 | 글자 #000000, 배경 #FFFFFF/80%, 테두리 #FFFFFF 적용 | ⬜ (Sprint 2) |
| 테마 전환 | 토글 시 모든 색상 일괄 변경, 텍스트 내용 유지, 깜빡임 없음 | ⬜ (Sprint 2) |
| 컨텍스트 메뉴 완성 | PRD F11 전체 항목 접근 가능 (Click-Through 제외 — Phase 4) | ⬜ (Sprint 2) |
