...
## Sprint 1 상세 — 글로벌 핫키 + Click-Through ✅ 완료
...
| 항목 | 심각도 | 대응 방안 | Sprint |
|------|--------|----------|--------|
| ~~핫키 충돌 (다른 앱 선점)~~ | ~~⚠️ 중간~~ | ~~트레이 벌룬 알림 + Click-Through 기능 비활성화~~ | ~~Sprint 1~~ ✅ 해결 |
| ~~Click-Through "갇힘" (핫키/트레이 모름)~~ | ~~⚠️ 중간~~ | ~~첫 1회 벌룬 안내 + 트레이 아이콘 상태 변경~~ | ~~Sprint 1~~ ✅ 해결 |
| ~~전체 투명 + Click-Through 조합 위험~~ | ~~⚠️ 중간~~ | ~~투명도 하한 20% 강제~~ | ~~Sprint 1~~ ✅ 해결 |
| 다중 모니터 DPI 차이 | 낮음 | PRD 범위 밖, 백로그 | — |
| 앱 아이콘 디자인 | 낮음 | 기본 아이콘으로 시작, 추후 개선 | Sprint 2 |
| ~~Click-Through 상태에서 앱 재시작~~ | ~~낮음~~ | ~~재시작 시 Click-Through OFF 기본값 (안전 설계)~~ | ~~Sprint 1~~ ✅ 해결 |
| `BackgroundTransparentMenuItem_Click` null 체크 누락 | 낮음 | `sender as MenuItem` 패턴에서 null 가드 없음 — Phase 3 Sprint 2에서 이월된 이슈, 실제 런타임 위험 낮음 | Sprint 2에서 개선 권장 |
...
| 항목 | 기준 | 상태 |
|------|------|------|
| 글로벌 핫키 동작 | 다른 앱 포커스 상태에서 Ctrl+Shift+N으로 표시/숨김 토글 | ⬜ 수동 검증 필요 |
| Click-Through 동작 | Ctrl+Shift+T로 Click-Through 토글, 마우스 클릭이 뒤의 앱에 전달 | ⬜ 수동 검증 필요 |
| Click-Through 시각 피드백 | 빨간 점선 테두리 + 상단 바 텍스트 + 트레이 아이콘 변경 | ⬜ 수동 검증 필요 |
| Click-Through 안전장치 | 투명도 하한 20%, 핫키 실패 시 기능 비활성화, 재시작 시 OFF | ⬜ 수동 검증 필요 |
| 트레이 메뉴 연동 | Click-Through 토글 항목 동작 및 상태 표시 | ⬜ 수동 검증 필요 |
| 핫키 충돌 처리 | 등록 실패 시 트레이 벌룬 알림 표시 | ⬜ 수동 검증 필요 |
| 메모리 사용량 | 80MB 이하 (유휴 상태) | ⬜ Sprint 2 |
| 시작 시간 | 2초 이내 | ⬜ Sprint 2 |
| 유휴 CPU | 0%에 근접 | ⬜ Sprint 2 |
| DPI 인식 | app.manifest dpiAware 선언, Win10/11 정상 표시 | ⬜ Sprint 2 |
| 앱 아이콘 | EXE 파일 및 작업 표시줄에 아이콘 표시 | ⬜ Sprint 2 |
| 단일 EXE 배포 | 설치 없이 EXE만으로 실행 가능 | ⬜ Sprint 2 |
| Windows 10/11 지원 | 양쪽 OS에서 전체 기능 정상 동작 | ⬜ Sprint 2 |

> Sprint 1 완료: 2026-04-17, PR phase4-sprint1 → develop (수동 생성 필요)
